...
## 工夫した点
- 自分の各お気に入り一覧は他人が閲覧することは出来ない為、人目を気にすることなく登録することが出来ます
- コメント機能を撤廃することで他者からの称賛や同意は得られませんが、否定やマウントを受けることがありません
- レベルとアイコンの実装により、育成のような一種のゲーム要素を追加し、僅かながらの達成感を得られると考えました
- サイト内で使用されている画像（投稿画像を除く）を全て自作する事によって暖かみのあるデザインを実現しました


## 実装予定の機能
- スキンによる背景の変更　
- 季節限定スキンの実装　
- ポイント機能の実装とそれにより入手できるスキンの実装　
- Pay.jpを用いたプレミアムスキンの実装　
- 特定の条件を満たした場合に入手することができるスキンの実装  
...
## 注意事項
- 実際にご利用いただいているユーザーがいる為、目に余る投稿が映ずる可能性が有ります。その際は何卒ご容赦いただけますと幸いです。
